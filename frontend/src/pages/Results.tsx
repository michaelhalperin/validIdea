  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center bg-[#030303]">
        ...
        <p className="text-gray-400 animate-pulse">Initializing Analysis Engine...</p>
      </div>
    );
  }

  if (error || !ideaData) return <div className="text-center py-20 text-red-400">Error loading data</div>;
      ...
      <div className="min-h-screen flex flex-col items-center justify-center text-center bg-[#030303]">
        <div className="relative w-32 h-32 mb-8">
          ...
          <div className="absolute inset-0 flex items-center justify-center">
            <Sparkles className="w-12 h-12 text-[#6366F1] animate-pulse" />
          </div>
        </div>
        <h2 className="text-4xl font-bold mb-4 tracking-tight text-white">Analyzing Venture</h2>
        <p className="text-gray-400 max-w-md mx-auto text-lg">
          Calculating market vectors, simulating competitors, and generating strategic roadmap...
        </p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-4 bg-[#030303]">
        ...
        <div className="flex gap-4">
          <button onClick={() => navigate('/new-idea')} className="btn-ghost">New Idea</button>
          <button onClick={() => runAnalysisMutation.mutate()} className="btn-primary">Retry</button>
        </div>
      </div>
    );
  }
  ...
  if (ideaData.status === 'DRAFT') {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center text-center px-4 bg-[#030303]">
        ...
        >
          {runAnalysisMutation.isPending ? <Loader2 className="animate-spin" /> : <Sparkles />}
          Run Analysis Engine
        </button>
      </div>
    );
    ...
    <div className="flex h-screen bg-[#030303] overflow-hidden text-white font-sans selection:bg-[#6366F1]/30">
            ...
            {analysis?.confidenceOverall && (
              <div className="bg-white/5 rounded-2xl p-4 text-center relative overflow-hidden">
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-xs font-medium text-gray-400 mt-2">Venture Confidence Score</p>
                </div>
              </div>
            )}
            {/* Export Buttons */}
      ...
      <main className="flex-1 overflow-y-auto relative scroll-smooth">
        ...
        <div className="max-w-5xl mx-auto p-6 lg:p-12 relative z-10">
          {/* Header Section */}
          <div className="mb-10">
            <h2 className="text-3xl font-bold text-white mb-4">{navItems.find(i => i.id === activeTab)?.label}</h2>
            <p className="text-gray-400 text-lg font-light max-w-3xl">{ideaData.oneLiner}</p>
          </div>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="p-8 rounded-3xl bg-gradient-to-br from-white/5 to-transparent border border-white/10 backdrop-blur-xl">
                  <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
                    <Sparkles className="w-5 h-5 text-[#6366F1]" /> Executive Summary
                  </h3>
                ...
                {analysis.keywords && (
                  <div className="flex flex-wrap gap-2">
                    {analysis.keywords.map((k, i) => (
                      <span key={i} className="px-3 py-1 rounded-full bg-white/5 border border-white/10 text-xs font-mono text-gray-400">
                        #{k}
                      </span>
                    ))}
                  </div>
          ...
          <div className="mt-20 pt-8 border-t border-white/5">
            ...
            {showRaw && (
              <div className="mt-4 p-6 rounded-2xl bg-black border border-white/5 overflow-x-auto">
                <pre className="text-xs font-mono text-gray-500">{JSON.stringify(analysis, null, 2)}</pre>
              </div>
            )}
          </div>
        </div>
